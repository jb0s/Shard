...
char* FortUpdater::fGetObjectName(uintptr_t Object)
{
	...
	FString objName;
	fGetNameByIdx(&index, &objName);

	if (objName.IsValid())
	{
		sName = objName.c_str();
		//this->FreeObjName((uintptr_t)objName.c_str());
	}
	//std::wstring sName(L"");
	//
	...
	//	FString objName;
	//	fGetNameByIdx(&index, &objName);
	//	//FString objName = std::wstring(std::string(this->fGetNameByIndex(index)).begin(), std::string(this->fGetNameByIndex(index)).end()).c_str();
	//
	//	if (objName.IsValid())
	//	{
